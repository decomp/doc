...
The final implementation of the subgraph isomorphism search algorithm is a cleaned up and thoroughly tested version of the non-concurrent library, which has a 94.8\% code coverage; as further described in section \ref{sec:ver_code_coverage}. The final implementation was developed in the \textit{``isobug''} branch on GitHub, and merged\footnote{Fix subgraph isomorphism search: \url{https://github.com/decomp/graphs/pull/1}} once stable with the \textit{``master''} branch.
