...
% ### problem
% The reason for the crash is that the list is identified such that the entry node is at the pre-test loop body and the exit node is at the pre-test loop header. When the primitive is merged it creates two top-level nodes which breaks the precondition of the dominator calculation algorithm.
% ### solution
% A potential fix would be to check the dominance info (e.g. entry has to dominate exit for valid candidates). This fix belongs in the iso package.
%
% * Further details of the crash in iso_crash_fix/
