...
\begin{enumerate}
	...
	\item Critically evaluate a set of Intermediate Representations (IRs), which describes low-, medium- and high-level language semantics, to identify one or more suitable for the decompilation pipeline. \label{ir_evaluation_obj}
...
% TODO: Mention that the following objective is prioritized as WON'T?
% 7) expression_lib_obj
...
\begin{table}[htbp]
	\begin{center}
		\begin{tabular}{|l|l|l|}
			...
			\hline
			\textbf{R1} & MUST & Describe common structural analysis techniques \\
			\textbf{R2} & SHOULD & Outline the general stages of the decompilation pipeline \\
			\textbf{R3} & COULD & Identify a suitable IR for the decompilation pipeline \\
			\hline
		\end{tabular}
	\end{center}
...
% TODO: Ref the following objectives:
% 1) decomp_research_obj
% 2) ir_evaluation_obj
...
\begin{table}[htbp]
	\begin{center}
		\begin{tabular}{|l|l|l|}
			...
			Req. ID & Priority & Description \\
			\hline
			\textbf{R4} & MUST & Read the assembly representation of LLVM IR \\
			\textbf{R5} & MUST & Write the assembly representation of LLVM IR \\
			\textbf{R6} & MUST & Interact with an in-memory representation of LLVM IR \\
			\textbf{R7} & MUST & Produce CFGs from LLVM IR basic blocks \\
			\textbf{R8} & COULD & Visualize CFGs using the \texttt{DOT} graph description language \\
			\textbf{R9} & WON'T & Read the bitcode representation of LLVM IR \\
			\textbf{R10} & WON'T & Write the bitcode representation of LLVM IR \\
			\textbf{R11} & WON'T & Provide a formal grammar of LLVM IR \\
			\hline
		\end{tabular}
	\end{center}
...
\begin{table}[htbp]
	\begin{center}
		\begin{tabular}{|l|l|l|}
			...
			Req. ID & Priority & Description \\
			\hline
			\textbf{R12} & MUST & Support analysis of reducible graphs \\
			\textbf{R13} & MUST & Recover pre-test loops (e.g. \texttt{for}, \texttt{while}) \\
			\textbf{R14} & MUST & Recover infinite loops (e.g. \texttt{while(TRUE)}) \\
			\textbf{R15} & MUST & Recover 2-way conditionals (e.g. \texttt{if}, \texttt{if-else}) \\
			\textbf{R16} & SHOULD & Recover post-test loops (e.g. \texttt{do-while}) \\
			\textbf{R17} & SHOULD & Recover n-way conditionals (e.g. \texttt{switch}) \\
			\textbf{R18} & COULD & Recover multi-exit loops \\
			\textbf{R19} & COULD & Recover nested loops \\
			\textbf{R20} & WON'T & Support analysis of irreducible graphs \\
			\textbf{R21} & WON'T & Recover compound boolean expressions \\
			\hline
		\end{tabular}
	\end{center}
...
\begin{table}[htbp]
	\begin{center}
		\begin{tabular}{|l|l|l|}
			...
			Req. ID & Priority & Description \\
			\hline
			\textbf{R22} & MUST & Perform structural decompilation passes on LLVM IR \\
			\textbf{R23} & MUST & Support language-agnostic interaction with other components \\
			\hline
		\end{tabular}
	\end{center}
...
% TODO: Ref the following objective:
% 6) reconstruct_tool_obj
