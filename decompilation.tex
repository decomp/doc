...
% TODO: Check that henceforth can be used to specify "from this point on".
...
The dynamic string table contains the names of libraries (e.g. \textit{``libc.so.6''}) and identifiers (e.g. \textit{``printf''}) which are required for dynamic linking. Other sections refer to these strings using offsets into \texttt{.dynstr}. The dynamic symbol table declares an array of dynamic symbol entities, each specifying the name (e.g. offset to \textit{``printf''} in \texttt{.dynstr}) and binding information (local or global) of a dynamic symbol. Both the \texttt{.plt} and the \texttt{.rela.plt} sections refers to these dynamic symbols using array indicies.

Lets take a step back for a moment and reflect on how dynamic linking is accomplished on a Linux system by reviewing the assembly instructions of the executable \texttt{.text} and \texttt{.plt} sections as outlined by figure \ref{elf_text} and \ref{elf_plt} respectively.

\begin{figure}[htbp]
	\lstinputlisting[language={[x86masm]Assembler},xleftmargin=1cm,xrightmargin=1cm]{inc/elf_text.asm}
	\caption{The assembly instructions of the \texttt{.text} section.}
	\label{elf_text}
\end{figure}

\begin{figure}[htbp]
	\lstinputlisting[language={[x86masm]Assembler},xleftmargin=1cm,xrightmargin=1cm]{inc/elf_plt.asm}
	\caption{The assembly instructions of the \texttt{.plt} section.}
	\label{elf_plt}
\end{figure}

As visualized in figure \ref{elf_text} the call instruction targets the \texttt{.printf} label of the PLT instead of the actual address of the \textit{printf} function in the \textit{libc.so.6} library. The Procedure Linkage Table (PLT) provides a level of indirection between call instructions and actual function (procedure) addresses, and contains one entity per external function as outlined in figure \ref{elf_plt}. The \texttt{.printf} entity of the PLT contains a jump instruction which targets the address stored in the \texttt{.printf} entity of the GOT. Initially this GOT entity contains the address of the next instruction, i.e. the instruction denoted by the \texttt{.resolve\_printf} label in the PLT.

% TODO: Refer to the trampolining method? (a method commonly known as trampolining)

%At first this entry contains the address

The \texttt{.rela.plt} section specifies the relocation entities of the PLT. More specifically it informs the linker of the virtual address to the \textit{printf} and \textit{exit} functions in the GOT.

%Upon execution the kernel maps the segments of the executable to pages in memory with appropriate access permissions.


%call to \textit{printf} is made using a level of indirection. Instead of directly refering to the address of the \textit{printf} function in \textit{libc.so.6} the call instruction targets the \texttt{.printf} label of the \texttt{.plt} section.

%Taking a step back it is worth noting that the \textit(``printf'') and \textit{``exit''} functions of our \textit{``hello world''} executable are resolved during runtime using lazy dynamic linking. That is, they are not resolved when the executable is loaded but rather

%The \texttt{.rela.plt} section specifies the relocation entities of the Procedure Linkage Table (PLT). More specifically it contains the virtual address to the \textit{printf} and \textit{exit} functions in the Global Offset Table (GOT) of the PLT.

%To summarize, the execution of a dynamically linked executable can roughly be described as follows. First the kernel parses
